...
CREATE TABLE rate (id BIGINT AUTO_INCREMENT, user_rated_id BIGINT NOT NULL, user_rater_id BIGINT NOT NULL, trade_id BIGINT NOT NULL, feedback FLOAT(18, 2), satisfaction FLOAT(18, 2), credibility FLOAT(18, 2), text VARCHAR(150), created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, UNIQUE INDEX rate_uniq_idx (user_rated_id, user_rater_id, trade_id), INDEX user_rated_id_idx (user_rated_id), INDEX user_rater_id_idx (user_rater_id), INDEX trade_id_idx (trade_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE trade (id BIGINT AUTO_INCREMENT, have_from_id BIGINT, have_to_id BIGINT, user_1_id BIGINT, user_2_id BIGINT, state VARCHAR(255), created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX have_from_id_idx (have_from_id), INDEX have_to_id_idx (have_to_id), INDEX user_1_id_idx (user_1_id), INDEX user_2_id_idx (user_2_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE rate ADD CONSTRAINT rate_trade_id_trade_id FOREIGN KEY (trade_id) REFERENCES trade(id) ON DELETE CASCADE;
