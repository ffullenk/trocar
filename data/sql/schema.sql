...
CREATE TABLE rate (id BIGINT, intercambio_id BIGINT, user_rated_id BIGINT, user_rater_id BIGINT, feedback FLOAT(18, 2), satisfaction FLOAT(18, 2), credibility FLOAT(18, 2), text VARCHAR(150), created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, PRIMARY KEY(id, intercambio_id, user_rated_id, user_rater_id)) ENGINE = INNODB;
CREATE TABLE reputation (id BIGINT AUTO_INCREMENT, user_id BIGINT, reputation FLOAT(18, 2), intercambios BIGINT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, UNIQUE INDEX reputation_user_uniq_idx (user_id), PRIMARY KEY(id, user_id)) ENGINE = INNODB;
...
CREATE TABLE trade (id BIGINT AUTO_INCREMENT, have_from_id BIGINT NOT NULL, have_to_id BIGINT NOT NULL, state VARCHAR(255), created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX have_from_id_idx (have_from_id), INDEX have_to_id_idx (have_to_id), PRIMARY KEY(id)) ENGINE = INNODB;
